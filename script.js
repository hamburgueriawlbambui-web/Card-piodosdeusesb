...
const MENU = {
  ...
  porcoes: [
    ...
    { id: "p-anel-cebola", name: "Porção Anel de Cebola", desc: "Porção anel de cebola.", price: 20.00, img: "img-anel-cebola.jpg" },
  ],
  bebidas: [
...
function isPickup(){
  ...
  return checked && checked.value === "retirada";
}
function currentDeliveryFee(){
  return isPickup() ? 0 : DELIVERY_FEE;
}
function cartCount(){
...
function cartTotal(){
  ...
  return items > 0 ? items + currentDeliveryFee() : 0;
}
...
function renderCart(){
  ...
  document.getElementById("cartDelivery").textContent = pickup ? "Grátis (retirada)" : money(count > 0 ? DELIVERY_FEE : 0);
  document.getElementById("cartTotal").textContent = money(cartTotal());
  document.getElementById("deliveryNote").style.display = pickup ? "none" : "block";
  document.getElementById("custAddress").required = !pickup;
  document.getElementById("custAddress").style.display = pickup ? "none" : "block";
  document.getElementById("custReference").style.display = pickup ? "none" : "block";
...
function checkout(){
  ...
  if(!name || !payment || (!pickup && !address)){
    showToast(pickup ? "Preencha nome e forma de pagamento" : "Preencha nome, endereço e forma de pagamento");
    return;
  }
  ...
  msg += `\n*${pickup ? "Retirada no local" : "Dados para entrega"}:*`;
  msg += `\nNome: ${name}`;
  if(!pickup){
    msg += `\nEndereço: ${address}`;
    if(reference){
